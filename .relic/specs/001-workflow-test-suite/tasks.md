...
- [ ] **T-01a** Add `"test": "bun test src/__tests__"` to the `scripts` section of `packages/core/package.json`
- [ ] **T-01b** Add `"test": "bun run --filter '*' test"` to the `scripts` section of root `package.json`

---

## Phase 2 — Pure unit tests

- [ ] **T-02** Create `packages/core/src/__tests__/spec-id.test.ts`
...
- [ ] **T-03** Create `packages/core/src/__tests__/intersection.test.ts`
...
- [ ] **T-04** Create `packages/core/src/__tests__/fs.test.ts`
...
- [ ] **T-05** Create `packages/core/src/__tests__/changelog.test.ts`
...
- [ ] **T-06** Create `packages/core/src/__tests__/context-builder.test.ts`
...
- [ ] **T-07** Create `packages/core/src/__tests__/init.test.ts`
...
- [ ] **T-08** Create `packages/core/src/__tests__/use.test.ts`
  - Writes specId to `.relic/current-spec`
  - Second call with different specId overwrites the first

- [ ] **T-09** Create `packages/core/src/__tests__/scaffold.test.ts`
...
- [ ] **T-11** Create `packages/core/src/__tests__/context.test.ts`
...
- [ ] **T-12** Create `packages/core/src/__tests__/search.test.ts`
...
- [ ] **T-13** Create `packages/core/src/__tests__/scan.test.ts`
...
- [ ] **T-14** Create `.github/workflows/test.yml`
...
- **Task ordering**: T-01 must be done first (establishes the test command). T-02 through T-13 are independent of each other and can be done in any order within their phase. T-14 is independent of all test files.
